...
const personTemplate = (option) => {
  ...
  return (
      <div style={{height:"2rem"}} className="flex align-items-center">
          ...
          <i onClick={()=>{handleOpenProfile(id)}} className="pi pi-user" style={{ fontSize: '1.5rem' }}></i>
          &nbsp;&nbsp;<i onClick={()=>{handleOpenChat(id)}} className="pi pi-comment p-overlay-badge" style={{ fontSize: '1.5rem' }}>
          { option.unseen ? 
          <Badge size={"small"} value={option.unseen} severity="danger"></Badge> 
          : ""  
          }
          </i>
      </div>
